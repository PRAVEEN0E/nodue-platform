...
import { getHodStudentStatus } from "@/lib/hod-api";
...
export default function HodStudentStatusPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const snap = await getHodStudentStatus(studentId);
      setStatus(snap);
    } catch (err) {
  ...
  if (error || !status) {
    return (
      <div>
        <PageHeader breadcrumb="HOD / Students / Status" title="Clearance Status" description="Student clearance pipeline" />
  ...
  return (
    <div>
      <PageHeader
        breadcrumb="HOD / Students / Clearance"
        title="Student Clearance Progress"
        description="Full clearance pipeline status for this student."
        actions={
